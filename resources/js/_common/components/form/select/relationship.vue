    ...
    export default {
        ...
        methods: {
            ...
            _setupListeners: function() {
                ...
                vueEventBus.$on('select2-selected-single', function(id, relationship) {
                    if (id == self.id) {
                        vueEventBus.$emit('selected-relationship', id, relationship);
                    }
                });
            ...
            getModel: function() {
                ...
                return axios.get('/api/v1/relationship/type')
                    .then(function(response) {

                        var options = [];

                        _.each(response.data.payload, function(relationship) {
                            options.push({
                                id: relationship.id,
